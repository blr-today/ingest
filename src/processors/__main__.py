...
def discover_processors() -> List[Type[Processor]]:
    ...
    for importer, modname, ispkg in pkgutil.iter_modules(processors_package.__path__, processors_package.__name__ + "."):
        ...
        try:
            ...
            for name in dir(module):
                ...
                if (isinstance(obj, type) and 
                    issubclass(obj, Processor) and 
                    obj is not Processor):
...
def process_events():
    ...
    logger.info(f"Discovered {len(processors)} processors: {[p.__name__ for p in processors]}")
            ...
            for processor_class in processors:
                if should_process_url(processor_class, url):
                    try:
                        ...
                        logger.error(f"Error in processor {processor_class.__name__} for URL {url}: {e}")
