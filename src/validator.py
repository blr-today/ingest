...
class SchemaOrgValidator:
    ...
    EVENT_SCHEMA = {
        ...
        "type": "object",
        "properties": {
            "@context": {"type": "string", "pattern": ".*schema\\.org.*"},
            "@type": {
                "type": "string",
                "enum": [
                    "Event",
    ...
    def validate_event(
        self, event_data: Dict[str, Any], url: str = None
    ) -> ValidationResult:
...
def write_github_step_summary(stats: Dict[str, int], results: List[tuple]) -> None:
    ...
    with open(github_step_summary, "w") as f:
        ...
        f.write(
            f"- **Valid Events**: {stats['valid']} ({stats['valid'] / stats['total'] * 100:.1f}%)\n"
        )
        f.write(
            f"- **Invalid Events**: {stats['invalid']} ({stats['invalid'] / stats['total'] * 100:.1f}%)\n"
        )
        f.write(
        ...
        warning_results = [
            result for _, result in results if result.is_valid and result.warnings
        ]
        if warning_results:
...
def validate_all_events(output_file: str = None, verbose: bool = False):
    ...
    logger.info(
        f"📊 Total: {stats['total']}, Valid: {stats['valid']} ({stats['valid'] / stats['total'] * 100:.1f}%), "
            ...
            json.dump(
                {
                    ...
                    "results": [
                        ...
                        or result.warnings  # Only log invalid events or events with warnings
                    ],
                },
                f,
                indent=2,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Schema.org Event validator using PyLD + jsonschema"
    )
