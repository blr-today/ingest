...
def get_cached_session(cache_name='event-fetcher-cache', days=1, allowable_codes=(200,), allowable_methods=["GET"], backend="sqlite", serializer=None):
    """
...
class Fetch():
    ...
    def _send_and_cache(
        ...
        response = curl.request(method=request.method, url=request.url, headers=request.headers, data=request.body, impersonate=self.browser)
    ...
    def request(self, method, cache=True, **kwargs):
        if self.browser:
            ...
            if cache:
                ...
                response = curl.request(method=method, impersonate=self.browser, **kwargs)
