...
def extract_timing(body_html):
    ...
    if timing_match:
        ...
        timing_match = re.search(r"(\d{1,2})\s+(AM|PM)", clean_text, re.IGNORECASE)
        if timing_match:
            start_hour, start_period = timing_match.groups()
            start_hour = int(start_hour) % 12 + (
                12 if start_period.upper() == "PM" else 0
            )
...
def guess_location(body_html):
    ...
    for line in body_text.split("\n"):
        if (
            "Bengaluru" in line
            or "Koramangala" in line
            or "Blue Tokai Coffee Roasters" in line
        ):
...
def generate_event_object(product_json, variant, date, start_hour, end_hour):
    ...
    if not location:
        print(
            f"[BLUETOKAI] Could not find a location in BLR: https://bluetokaicoffee.com/products/{product_json['product']['handle']}"
        )
        return None
