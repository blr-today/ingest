...
def fetch_event_detail(slug, session):
    ...
    try:
        ...
        event_data = data["appsWarmupData"]["140603ad-af8d-84a5-2c80-a0f60cb47351"][
...
def create_recurring_events(base_event, start_dt, end_dt):
    ...
    while current_date <= end_date:
        ...
        day_start = datetime.combine(current_date, start_time).replace(
        ...
        if start_dt.date() != end_dt.date():
            event_copy["name"] = (
                f"{event_copy['name']} - {current_date.strftime('%b %d')}"
            )
    ...
    try:
        ...
        if "startDate" in scheduling:
            start_date = datetime.fromisoformat(
                scheduling["startDate"].replace("Z", "+00:00")
            )
        ...
        if "endDate" in scheduling:
            end_dt = datetime.fromisoformat(
                scheduling["endDate"].replace("Z", "+00:00")
            )
        ...
        if is_external and "external" in registration:
            ...
            base_event["offers"] = {
                ...
                if not ticketing.get("soldOut", False)
                else "SoldOut",
            }
        ...
        if start_dt and end_dt and start_dt.date() != end_dt.date():
            logger.info(
                f"Creating recurring events for multi-day event: {name} ({start_dt.date()} to {end_dt.date()})"
            )
        ...
        logger.error(
            f"Event URL: {EVENT_DETAIL_URL_PREFIX}{event.get('slug', 'unknown')}"
        )
        return None
...
def fetch_events():
    ...
    try:
        ...
        apps_data = data.get("appsWarmupData", {}).get(
                ...
                if isinstance(transformed, list):
                    for single_event in transformed:
                        ...
                        if single_event["url"] not in seen_urls:
                            ...
                            logger.debug(
                                f"Skipping duplicate event URL: {single_event['url']}"
                            )
                else:
                    ...
                    if transformed["url"] not in seen_urls:
                        ...
                        logger.debug(
                            f"Skipping duplicate event URL: {transformed['url']}"
                        )

        logger.info(
            f"Total transformed events after deduplication: {len(transformed_events)}"
        )
