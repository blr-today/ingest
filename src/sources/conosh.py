...
def parse_event_page(event_url):
    ...
    description = (
        f"Culinary Event organized by Conosh. See {event_url} for more details"
    )
    ...
    for menu_div in menu_items:
        ...
        date_match = re.search(
            r"(Aug|Sep|Oct|Nov|Dec|Jan|Feb|Mar|Apr|May|Jun|Jul)\s+(\d+)", item_text
        )
        if date_match:
            date_str = (
    ...
    for date_str, offers in date_groups.items():
        try:
            ...
            if time_matches:
                ...
                event_date = dateutil.parser.parse(f"{date_str} {event_time}").replace(
                    tzinfo=IST
                )
