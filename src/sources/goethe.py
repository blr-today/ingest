    ...
    if "Goethe-Institut" not in location_name:
        for line in response.text.splitlines():
            if '"event_location"' in line:
                startIndex = (
                    line.index('"event_location":') + len('"event_location":') + 1
                )
                endIndex = line.index('"', startIndex + 1)
                location_address = (
                    line[startIndex:endIndex].strip().replace("<br />", " ")
                )
...
def make_event(session, event_dict):
    ...
    details = {
        ...
        "location": {"name": event_dict["location_IDtxt"]},
        "startDate": startTime.isoformat(),
        "endDate": endTime.isoformat(),
        "keywords": ["GOETHE", event_dict.get("subheadline", "")]
        + [
            category["category_text"]
            for category in event_dict.get("secondary_categories", [])
        ],
        "@type": get_event_type(
            event_dict["event_type"], event_dict.get("subheadline", "")
        ),
...
def fetch_events():
    ...
    response = session.get(
        "https://www.goethe.de/rest/objeventcalendarRedesign/events/fetchEvents",
        params=payload,
    )
